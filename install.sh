...
if [ -z "$IP_ALLA" ] || [ -z "$IP_ALLB" ]; then
  echo "[ERROR] Không lấy được IP enX0"
  exit 1
fi
...
# urnetwork
docker run -d --network my_network_1  --name urnetwork1 --restart always  --cap-add NET_ADMIN --dns 8.8.8.8 --dns 1.1.1.1 -v ur_data1:/var/lib/vnstat -e USER_AUTH='nguyenvinhcao123@gmail.com' -e PASSWORD='CAOcao123CAO@' techroy23/docker-urnetwork:latest
docker run -d --network my_network_2  --name urnetwork2 --restart always  --cap-add NET_ADMIN --dns 8.8.8.8 --dns 1.1.1.1 -v ur_data2:/var/lib/vnstat -e USER_AUTH='nguyenvinhcao123@gmail.com' -e PASSWORD='CAOcao123CAO@' techroy23/docker-urnetwork:latest
