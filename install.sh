...
echo "[INFO] IP_ALLA=$IP_ALLA | IP_ALLB=$IP_ALLB"

# ===== docker-apps-start.sh =====
cat > "$START_SH" << "EOSH"
#!/usr/bin/env bash
set -Eeuo pipefail
log(){ echo "[`date +%F_%T`] $*"; }
...
# Start containers (MYST, TM, Repocket, EarnFM, PacketSDK, URNetwork)
docker run -d --network "$NET1" --cap-add NET_ADMIN -p "${IP_ALLA}:4449:4449" --name myst1 -v myst-data1:/var/lib/mysterium-node --restart unless-stopped mysteriumnetwork/myst:latest service --agreed-terms-and-conditions
docker run -d --network "$NET2" --cap-add NET_ADMIN -p "${IP_ALLB}:4449:4449" --name myst2 -v myst-data2:/var/lib/mysterium-node --restart unless-stopped mysteriumnetwork/myst:latest service --agreed-terms-and-conditions
docker run -d --network "$NET1" --restart=always --name tm1 traffmonetizer/cli_v2:arm64v8 start accept --token "$TM_TOKEN"
docker run -d --network "$NET2" --restart=always --name tm2 traffmonetizer/cli_v2:arm64v8 start accept --token "$TM_TOKEN"
docker run -d --network "$NET1" --restart=always --name repocket1 -e RP_EMAIL="$RP_EMAIL" -e RP_API_KEY="$RP_KEY" repocket/repocket:latest
docker run -d --network "$NET2" --restart=always --name repocket2 -e RP_EMAIL="$RP_EMAIL" -e RP_API_KEY="$RP_KEY" repocket/repocket:latest
docker run -d --network "$NET1" --restart=always --name earnfm1 -e EARNFM_TOKEN="$EARNFM_TOKEN" earnfm/earnfm-client:latest
docker run -d --network "$NET2" --restart=always --name earnfm2 -e EARNFM_TOKEN="$EARNFM_TOKEN" earnfm/earnfm-client:latest
docker run -d --network "$NET1" --restart unless-stopped --name packetsdk1 packetsdk/packetsdk -appkey=BFwbNdFfwgcDdRmj
docker run -d --network "$NET2" --restart unless-stopped --name packetsdk2 packetsdk/packetsdk -appkey=BFwbNdFfwgcDdRmj
docker run -d --network "$NET1" --restart=always --cap-add NET_ADMIN --name ur1 -e USER_AUTH="$UR_USER" -e PASSWORD="$UR_PASS" "$URN_IMAGE"
docker run -d --network "$NET2" --restart=always --cap-add NET_ADMIN --name ur2 -e USER_AUTH="$UR_USER" -e PASSWORD="$UR_PASS" "$URN_IMAGE"

log "✅ All Docker apps started."
EOSH
chmod +x "$START_SH"

# ===== apps-daily-refresh.sh =====
cat > "$REFRESH_SH" << "EOF"
#!/usr/bin/env bash
set -Eeuo pipefail
log(){ echo "[$(date +%F_%T)] $*"; }
...
docker run -d --network "$NET1" --name repocket1 -e RP_EMAIL="$RP_EMAIL" -e RP_API_KEY="$RP_KEY" --restart=always repocket/repocket:latest
docker run -d --network "$NET2" --name repocket2 -e RP_EMAIL="$RP_EMAIL" -e RP_API_KEY="$RP_KEY" --restart=always repocket/repocket:latest
docker run -d --network "$NET1" --restart=always -e EARNFM_TOKEN="$EARNFM_TOKEN" --name earnfm1 earnfm/earnfm-client:latest
docker run -d --network "$NET2" --restart=always -e EARNFM_TOKEN="$EARNFM_TOKEN" --name earnfm2 earnfm/earnfm-client:latest
...
[Service]
Type=oneshot
ExecStartPre=/usr/sbin/iptables -t nat -C POSTROUTING -s 192.168.33.0/24 -j SNAT --to-source $IP_ALLA
ExecStartPre=/usr/sbin/iptables -t nat -C POSTROUTING -s 192.168.34.0/24 -j SNAT --to-source $IP_ALLB
ExecStart=$START_SH
RemainAfterExit=yes
...
# ===== apps-daily-refresh.service + timer =====
...
echo "👉 Xem log: journalctl -u docker-apps.service -e --no-pager"
