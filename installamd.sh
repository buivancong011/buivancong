...
# ==== Nếu có container đang chạy -> xóa hết ====
...
# ==== Thiết lập iptables ban đầu (đổi sang enX0) ====
...
# ==== THÊM FIX IPTABLES SAU REBOOT (interface enX0) ====
cat <<'EOF' | sudo tee /usr/local/bin/fix_iptables.sh
#!/bin/bash
set -euo pipefail

IP_ALLA=$(/sbin/ip -4 -o addr show scope global enX0 | awk '{gsub(/\/.*/,"",$4); print $4}' | head -n1)
IP_ALLB=$IP_ALLA
...
# ==== Chạy các container (URNetwork đổi sang amd64) ====
...
# ⚠️ Khác biệt quan trọng: URNetwork → amd64
